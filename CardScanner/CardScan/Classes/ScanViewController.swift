...
@objc public class CreditCard: NSObject {
    ...
    @objc public func expiryFromCard() -> String? {
        guard var month = self.expiryMonth, var year = self.expiryYear else {
            return nil
        }
...
@objc public class ScanViewController: ScanBaseViewController {
    ...
    @objc static public func createViewController(withDelegate delegate: ScanDelegate? = nil, configuration: ScanConfiguration) -> ScanViewController? {
        ...
        let storyboard = UIStoryboard(name: "CardScan", bundle: BundleURL.bundle())
    ...
    func setUiCustomization() {
        ...
        self.cornerBorderColor = UIColor.gray.cgColor
    ...
    public override func viewDidLoad() {
        super.viewDidLoad()
        self.setStrings()
        self.denyPermissionTitle = "need_camera_access".localize()
        self.denyPermissionMessage = "need_camera_access_desc".localize()
        self.denyPermissionButtonText = "ok".localize()
        self.scanCardLabel.text = "scan_card".localize()
        self.positionCardLabel.text = "card_position_desc".localize()
        self.skipButton.setTitle("manually_enter".localize(), for: .normal)
        self.backButton.setTitle("back".localize(), for: .normal)
    ...
    override public func onScannedCard(number: String, expiryYear: String?, expiryMonth: String?, scannedImage: UIImage?) {
        ...
        if expiryMonth?.count == 1 {
            card.expiryMonth = "0\(expiryMonth ?? "")"
        }else{
            card.expiryMonth = expiryMonth
        }
        
        card.expiryYear = String((expiryYear ?? "").suffix(2))
