...
@objc open class ScanBaseViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate, ScanEvents, AfterPermissions {
    ...
    private var checkBrightness: Bool = true
    ...
    public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        if self.checkBrightness {
            self.checkBrightness = false
            self.getBrightness(sampleBuffer: sampleBuffer)
        }
    ...
    public func getBrightness(sampleBuffer: CMSampleBuffer) {
        ...
        if brightnessValue <= 1.2 {
            ...
            print("---TEST---- SCANNER --- LOW LIGHT \(brightnessValue)")
        } else {
            if self.isTorchOn() {
                self.toggleTorch()
            }
            print("---TEST---- SCANNER --- ENOUGH LIGHT \(brightnessValue)")
        }
